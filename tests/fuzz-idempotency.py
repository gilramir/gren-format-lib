...
def known_upstream_issue(workdir, source):
    ...
    Today there are two. A third, **compiler-common#14**'s block term applied
    to a token a multi-line comment pushed left of its body, is gone: the fork's
    parser applies no `if`, `when`, `let` or lambda to an argument, as the
    Haskell grammar cannot (geng-lang `m1a-fmt.md` §F19), so those probes are
    refused by the parser and skipped, as `geng make` refuses them.

# Development history (archived long-form `CLAUDE.md`)

**This file is an archive, not a spec.** Up to 2026-08-11 the package's
`CLAUDE.md` accumulated a running account of every gate that was built, every
bug it found, every fix, and — most usefully — every fix that was *tried and
backed out*. That narrative grew to 2,400 lines and was moved here whole so
`CLAUDE.md` could go back to being a short orientation file.

Read it when you want the **reasoning behind a rule** or want to know whether an
approach has already been tried. For current instructions, read `CLAUDE.md`; for
current behaviour, read `docs/formatterRules.md`, `docs/commentHandling.md` and
`docs/testing.md` — where any of those disagree with this file, they win. Counts
and sweep numbers below are the numbers *of the day they were written*.
